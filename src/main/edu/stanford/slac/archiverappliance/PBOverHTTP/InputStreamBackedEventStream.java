...
public class InputStreamBackedEventStream implements EventStream, RemotableOverRaw {
    ...
    public InputStreamBackedEventStream(InputStream is, Instant startTime) throws IOException {
        ...
        try {
            ...
            try {
                ...
                logger.error("Exception resetting mark in input stream", t);
            }
        }
    }

    public InputStreamBackedEventStream(
            InputStream is, Instant startTime, RetrievalEventProcessor retrievalEventProcessor) throws IOException {
        this(is, startTime);
    ...
    public Iterator<Event> iterator() {
        // The input stream has state. We could use a TeeInputStream to support multiple iterators but for now, we
        // insist on one iterator per event stream.

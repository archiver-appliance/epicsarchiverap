...
public class SlicePBFile {
    ...
    public static void slicePBFile(
            Path srcPath, Instant startTs, Instant endTs, Path destPath, PlainFileHandler handler) {
        logger.info("Slicing " + srcPath.toString()
                + " from "
                + TimeUtils.convertToISO8601String(startTs)
                + " to "
                + TimeUtils.convertToISO8601String(endTs)
                + " into "
                + destPath);
        try (ArchPaths contexts = new ArchPaths()) {
            try {
                FileInfo info = handler.fileInfo(srcPath);
                try (EventStream strm = handler.getStream(info.getPVName(), srcPath, info.getType());
                        EventFileWriter writer = handler.createEventFileWriter(
                                info.getPVName(), destPath, info.getType(), info.getDataYear())) {
                    int eventNumber = 0;
                    for (Event ev : strm) {
                        try {

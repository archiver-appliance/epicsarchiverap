...
public class LineEscaper {
    ...
    public static void escapeNewLines(byte[] input, OutputStream os) throws IOException {
        ...
        for (byte b : input) {
            switch (b) {
                case ESCAPE_CHAR:
                    os.write(ESCAPE_CHAR_SEQUENCE);
                    break;
                case NEWLINE_CHAR:
                    os.write(NEWLINE_CHAR_SEQUENCE);
                    break;
                case CARRIAGERETURN_CHAR:
                    os.write(CARRIAGERETURN_CHAR_SEQUENCE);
                    break;
                default:
                    os.write(b);
                    break;
    ...
    public static byte[] unescapeNewLines(byte[] input) {
        if (input == null) return null;
        try {
            // TODO if we eliminate raw byte arrays here, we can shave off about 1 sec out of 17 secs or so for 1Hz 1
            // year data.
                ...
                if (b == ESCAPE_CHAR) {
                    ...
                    switch (b) {
                        case ESCAPE_ESCAPE_CHAR:
                            bos.write(ESCAPE_CHAR);
                            break;
                        case NEWLINE_ESCAPE_CHAR:
                            bos.write(NEWLINE_CHAR);
                            break;
                        case CARRIAGERETURN_ESCAPE_CHAR:
                            bos.write(CARRIAGERETURN_CHAR);
                            break;
                        default:
                            bos.write(b);
                            break;

...
public class ValidateAndFixPBFile {
	...
	public static void fixPBFile(Path path, boolean verboseMode, boolean makeBackups) {
		...
		try (ArchPaths contexts = new ArchPaths()) {
			...
			if(verboseMode) logger.info("Temporary file is " + tempPath.toString() + " with final component of temp file " + tempFileName);
			try { 
					...
					for(Event ev : strm) {
						try {
							...
							if(epochSeconds >= previousEpochSeconds) {
								...
								if(verboseMode) logger.debug("Skipping non sequential event " + eventnum + " in file " + path.toString());
								skippedEvents++;
							}
						} catch(Throwable t) { 
				...
				if(makeBackups) { 
					Path tPath = Files.createTempFile(path.getParent(), "Temp", "tempbak", PosixFilePermissions.asFileAttribute(Files.getPosixFilePermissions(path, LinkOption.NOFOLLOW_LINKS)));

...
public class PvaGetPVData implements PvaAction {
	...
	public void request(PVStructure args, RPCResponseCallback callback, ConfigService configService) {
		...
		reqParameters.put("pv", "XF:31IDA-OP{Tbl-Ax:X1}Mtr.RBV");
		try {
			doGetSinglePV(reqParameters, callback, configService);
	...
	private void doGetSinglePV(Map<String, String> reqParameters, RPCResponseCallback resp, ConfigService configService) throws ServletException, IOException {
...
//		if(typeInfo == null && RetrievalState.includeExternalServers(reqParameters)) {
...
//			resp.sendError(HttpServletResponse.SC_NOT_FOUND);
		...
		Structure resultStructure = fieldCreate.createStructure("NTComplexTable", new String[] { "labels", "value" },
				new Field[] { fieldCreate.createScalarArray(ScalarType.pvString), fieldCreate.createStructureArray(
						NTScalar.createBuilder().value(ScalarType.pvDouble).addAlarm().createStructure()) });
		PVStructure result = PVDataFactory.getPVDataCreate().createPVStructure(resultStructure);
		
		PVStructureArray p = result.getStructureArrayField("value");
		try(BasicContext retrievalContext = new BasicContext(typeInfo.getDBRType(), pvNameFromRequest); 
				PvaMergeDedupConsumer mergeDedupCountingConsumer = createMergeDedupConsumer(p, useChunkedEncoding);
			...
			LinkedList<Future<RetrievalResult>> retrievalResultFutures = resolveAllDataSources(pvName, typeInfo, postProcessor, applianceForPV, retrievalContext, executorResult);
			...
			logger.info("Done with the RetrievalResult's; moving onto the individual event stream from each source for " + pvName);
			...
			for(Future<EventStream> future : eventStreamFutures) {
				...
				try(EventStream eventStream = future.get()) {
					...
					logger.info("Processing event stream for pv " + pvName + " from source " + ((eventStream.getDescription() != null) ? eventStream.getDescription().getSource() : " unknown"));
					...
					if(currentlyProcessingPV == null || !currentlyProcessingPV.equals(pvName)) {
						logger.info("Switching to new PV " + pvName + " In some mime responses we insert special headers at the beginning of the response. Calling the hook for that");
...
//							resp.flushBuffer();
					...
					if(ex != null && ex.toString() != null && ex.toString().contains("ClientAbortException")) {
						// We check for ClientAbortException etc this way to avoid including tomcat jars in the build path.
						logger.info("Exception when consuming and flushing data from " + (sourceDesc != null ? sourceDesc.getSource() : "N/A"), ex);
					} else { 
...
//					resp.flushBuffer();
			...
			resp.requestDone(StatusFactory.getStatusCreate().getStatusOK(), result);
	 ...
	 * @throws IOException
	 */
	private boolean parseTimeRanges(RPCResponseCallback resp, String pvName, LinkedList<TimeSpan> requestTimes, String timeRangesStr) throws IOException {
...
//					resp.sendError(HttpServletResponse.SC_BAD_REQUEST, msg);
		...
		System.out.println("COMPLETED calculating the time");
		return true;
	}
	 ...
	 */
	private PvaMergeDedupConsumer createMergeDedupConsumer(PVStructureArray p, boolean useChunkedEncoding)
			throws ServletException {
		...
		try {
			...
			// }
			System.out.println("CREATE PVA mergededup");
			...
			mergeDedupCountingConsumer = new PvaMergeDedupConsumer(mimeresponse, p);
	 ...
	 * @return
	 */
	private static RetrievalExecutorResult determineExecutorForPostProcessing(String pvName, PVTypeInfo typeInfo, LinkedList<TimeSpan> requestTimes, PostProcessor postProcessor) {
		long memoryConsumption = postProcessor.estimateMemoryConsumption(pvName, typeInfo, requestTimes.getFirst().getStartTime(), requestTimes.getLast().getEndTime(), null);
	...
	private LinkedList<Future<RetrievalResult>> resolveAllDataSources(String pvName, PVTypeInfo typeInfo, 
			...
			BasicContext retrievalContext, RetrievalExecutorResult executorResult) throws IOException {
		...
		for(TimeSpan timespan : executorResult.requestTimespans) { 
			...
			LinkedList<UnitOfRetrieval> unitsofretrieval = datasourceresolver.resolveDataSources(pvName, timespan.getStartTime(), timespan.getEndTime(), typeInfo, retrievalContext, postProcessor, null, null, applianceForPV);

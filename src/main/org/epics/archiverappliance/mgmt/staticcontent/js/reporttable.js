...
function createReportTable(jsonurl, tabledivname, coldefs, rowdefs) {
	...
	var reportTable = $('<table id="' + tabledivname + '_table" class="reporttable">');
	reportTable.append($('<thead>')
			.append($('<tr class="toolbar">').append($('<th colspan="3">').append(
	...
	if(rowdefs != null && rowdefs != undefined) reportTable.data('rowdefs', rowdefs);
...
function addRowsForData(tableobj) {
	...
	if(dataobjs.length == 0) {
		tableobj.append('<tr><td colspan="' + coldefs.length + '">No data found</td></tr>');
	}

	for(var r=curpage*pagesize; (r < dataobjs.length) && (r < (curpage+1)*pagesize); r++) {
...
function addSorterToTableTh(reportTable, indexofthelement) {
	...
	if(coldef.sortType != null && coldef.sortType != undefined && coldef.sortType == 'none') {
	...
	th.click(function() {
		...
		if(coldefs[indexofthelement].sortType != null && coldefs[indexofthelement].sortType != undefined) {
			sortingfnprefix = coldefs[indexofthelement].sortType + '_';
			if(sortingfunctions[sortingfnprefix+'asc'] == undefined || sortingfunctions[sortingfnprefix+'asc'] == null
					|| sortingfunctions[sortingfnprefix+'desc'] == undefined || sortingfunctions[sortingfnprefix+'desc'] == null
					) {
				alert("No sorting function defined for prefix " + sortingfnprefix + ". Defaulting to string");
		...
		success: function(data, textStatus, jqXHR) {
			...
			if(rowdefs != null) { 
				...
				if(initialSort != null && initialSort != undefined) {
					...
					var sortingfnprefix = "string_";
					if(coldefs[initialSort].sortType != null && coldefs[initialSort].sortType != undefined) {
					...
					if(initialSortDirection != null && initialSortDirection != undefined) {
						sortfunction = sortingfunctions[sortingfnprefix + 'asc'];
					}
...
function updateToolbarPageNum(reportTable) {
	...
	var toolBarPageNumSpan = reportTable.find('thead tr.toolbar span:eq(0) span:eq(2)').first();
...
function setupToolbarActions(reportTable) {
	...
	var pageNumSelector = reportTable.find('thead tr.toolbar select:eq(0)').first();

...
public class GetPVsForThisAppliance implements BPLAction {
	private static Logger logger = Logger.getLogger(GetPVsForThisAppliance.class.getName());

	@Override
	public void execute(HttpServletRequest req, HttpServletResponse resp, ConfigService configService) throws IOException {
		logger.debug("Getting pvs for appliance " + configService.getMyApplianceInfo().getIdentity());
		...
		try (PrintWriter out = resp.getWriter()) {
			...
			logger.error("Exception getting pvs for appliance " + configService.getMyApplianceInfo().getIdentity(), ex);

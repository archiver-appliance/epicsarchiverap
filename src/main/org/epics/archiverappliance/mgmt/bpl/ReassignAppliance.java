	...
	public void execute(HttpServletRequest req, HttpServletResponse resp, ConfigService configService) throws IOException {
		...
		for(String pvName: pvNames) {
			...
			if (srcApplianceInfo.getIdentity().equals(destApplianceInfo.getIdentity())) { 
				// The various components react to changes in the PVTypeInfoi. THis is not a change; so nothing will happen even if we pwemitted this.
				statuses.put(pvNameFromRequest, "Attempting to reassign onto same appliance - " + srcApplianceInfo.getIdentity() + "/" + destApplianceInfo.getIdentity());
				continue;
			}

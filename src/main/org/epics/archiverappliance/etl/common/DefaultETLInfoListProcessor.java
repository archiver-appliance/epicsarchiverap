...
public class DefaultETLInfoListProcessor extends ETLInfoListProcessor {
    ...
    protected ETLInfoListStatistics process(
        ...
        for (ETLInfo infoItem : etlInfoList) {
            ...
            if (notEnoughFreeSpace(sizeOfSrcStream, curETLDest, lookupItem, infoItem.getKey(), pvName)) {
                if (deleteSrcStreamWhenOutOfSpace(
                        List.of(infoItem), lookupItem.getOutOfSpaceHandling(), movedList, lookupItem, pvName)) {
                    continue;
                } else {
                    break;
                }

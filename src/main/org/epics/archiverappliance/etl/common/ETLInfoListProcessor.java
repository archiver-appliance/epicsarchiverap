...
public abstract class ETLInfoListProcessor {
    ...
    public static boolean notEnoughFreeSpace(
            long sizeOfSrcStream, ETLDest destMetrics, ETLPVLookupItems lookupItem, String key, String pvName)
            throws IOException {

        if (sizeOfSrcStream > 0 && destMetrics instanceof StorageMetrics) {
            long freeSpace = ((StorageMetrics) destMetrics).getUsableSpace(lookupItem.getMetricsForLifetime());
